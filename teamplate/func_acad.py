...
def line(file, x1, y1, x2, y2, L='0_LINHA'):
    ...
    x2 = round(float(x2), 6);
    y2 = round(float(y2), 6)
    file.write('LINE ' + str(x1) + ',' + str(y1) + ' ' + str(x2) + ',' + str(y2) + ' \n')
...
def polyline(file, pontos, C='C', L='0_LINHA'):
    ...
    for i in range(int(len(pontos) / 2)):
        file.write(str(round(pontos[int(0 + 2 * i)], 4)) + ',' + str(round(pontos[int(1 + 2 * i)], 4)) + ' ')
    file.write(C + '\n')
...
def rectangle(file, W, x1, y1, x2, y2, L='0_LINHA'):
    ...
    x2 = round(float(x2), 6);
    y2 = round(float(y2), 6)
    file.write('RECTANG W ' + str(W) + ' ' + str(x1) + ',' + str(y1) + ' ' + str(x2) + ',' + str(y2) + '\n')
...
def mtext(file, x1, y1, x2, y2, texto, subtexto='', J='center', H=.0621, R=0):
    ...
    if subtexto == '':
        file.write(
            'MTEXT ' + str(x1) + ',' + str(y1) + ' H ' + str(H) + ' R ' + str(R) + ' L A  J MC ' + str(x2) + ',' + str(y2) + J + '\H' + str(H) + ';' + texto + '}' + '\n\n')
    else:
        file.write(
            'MTEXT ' + str(x1) + ',' + str(y1) + ' H ' + str(H) + ' R ' + str(R) + ' L A  J MC ' + str(x2) + ',' + str(y2) + J + '\H' + str(H) + ';' + texto + '}' + '\P{\H0.05;' + subtexto + '}\n\n')
...
def new_layer(file, nome, color, RGB, L, LW, D):
    ...
    RGB   -> 1  or                 R,G,B
    """
    file.write('-LAYER M ' + nome + ' ' + color + ' ' + RGB + ' ' + nome + ' L ' + L + ' ' + nome +
               ' LW ' + LW + ' ' + nome + ' D ' + D + '\n' + nome + ' plot plot ' + nome + '\n\n')
...
def new_block(file, name, bp_x, bp_y, x1, y1, x2, y2):
    ...
    file.write('-BLOCK ' + name + ' ' + str(bp_x) + ',' + str(bp_y) + ' ' + str(x1) + ',' + str(y1) + ' ' + str(x2) + ',' + str(y2) + ' \n')
...
def insert_block(file, name, bp_x, bp_y, scale_x='1', scale_y='1', rotation='0'):
    ...
    file.write(
        '-INSERT ' + name + ' ' + str(bp_x) + ',' + str(bp_y) + ' ' + str(scale_x) + ' ' + str(scale_y) + ' ' + str(rotation) + '\n')

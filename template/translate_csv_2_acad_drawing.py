...
# GUIDELINES TO ACAD SCRITPING LANGUAGE:
#  space is a less powerful "enter/new line"; "enter/new line" always finishes the input
#  "_" before a command -> command is run in English
#  "-" before a command -> CLI version (without prompt)
#
# To add new colors, update function "interpretar_cor" and add the "new_layer"
# To add new granulometries, create a new hatch in "PADRAO/HATCHES" and add it in "interpretar_granulometria"
"""

import codecs
import os

from func_acad import (circle, hatch, insert_block, line, mspace_zoom, mtext,
                       new_block, new_layer, new_style, polyline, psetup,
                       rectangle, set_layer, zoom)
from func_print import division, final_message, section  # printing functions
# 1. Import libraries ===================================================================
...
def read_files(title_block, borehole_data, output_scr_acad, logo_png):
    ...
    title_block = os.path.join(abs_path, title_block)
    borehole_data = os.path.join(abs_path, borehole_data)
    output_scr_acad = os.path.join(abs_path, output_scr_acad)
    logo_png = os.path.join(abs_path, logo_png)

    # Title block
    section(84, "1. Title block")
    INFO = pd.read_csv(title_block, sep=";", encoding="cp1252", header=None)
    ...
    for _ in range(len(CSV)):
        ...
        CSV.loc[_, "ID_sonda"] = (
            "" if CSV["ID_sonda"][_] == "nan" else CSV["ID_sonda"][_]
        )
...
def create_layers(file):
    ...
    new_layer(file, "0_SELO", "C", "cyan", "Continuous", "default", "Limites no layout")
    new_layer(
        file,
        "1_ACABAMENTO",
        "C T",
        ...
        "Continuous",
        "default",
        "Cor do acabamento e cap de pressão",
    )
    new_layer(
        file,
        "1_BORRACHA",
    ...
    new_layer(
        file,
        '"SÍMBOLO_COLETA AMOSTRA"',
        "C T",
        "240,0,0",
        "Continuous",
        "default",
        "Marcador da profundidade de coleta da amostra",
    )
    new_layer(
        file, '"H_CIMENTO"', "C", "253", "Continuous", "default", "Cor do cimento"
    )
    new_layer(
        file, '"H_BENTONITA"', "C", "23", "Continuous", "default", "Cor da bentonita"
    )
    new_layer(
        file,
        '"H_PRE_FILTRO"',
    ...
    new_layer(
        file, '"SC_BEGE"', "C T", "244,230,189", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file,
        '"SC_BRANCA"',
    ...
    new_layer(
        file, '"SC_CINZA"', "C T", "105,105,105", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file,
        '"SC_CINZA_CLARO"',
    ...
    new_layer(
        ...
        "Continuous",
        "default",
        "Cor do plano de fundo do gramado",
    )
    new_layer(
        file, '"SC_MARROM"', "C T", "76,57,38", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file,
        '"SC_MARROM_AVERMELHADO"',
        "C T",
        "132,41,0",
        "Continuous",
        "default",
    ...
    new_layer(
        file, '"SC_PRETO"', "C T", "40,15,16", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file, '"SC_ROXO"', "C T", "76,53,89", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file, '"SC_VERDE"', "C T", "69,107,59", "Continuous", "default", "Cor do solo"
    )
    new_layer(
        file,
        '"SC_VERMELHO"',
...
def create_blocks(file):
    zoom(file, -2, -2, 2, 2)
    L = '"SÍMBOLO_NÍVEL ÁGUA"'
    line(file, 0, 0, 0.1375, 0, L=L)
    line(file, 0, 0, 0.075, 0, L=L)
    line(file, 0.0094, -0.0188, 0.0656, -0.0188, L=L)
    line(file, 0.0187, -0.0375, 0.0562, -0.0375, L=L)
...
def validate(CSV, n_p):
    if (
        np.isnan(CSV["Topo_liso"][n_p])
        and np.isnan(CSV["C_liso"][n_p])
        and np.isnan(CSV["C_sec_filtr"][n_p])
    ):
        existe_poco = 0
    else:
        existe_poco = 1
        # CONSISTIR PROFUNDIDADE DE SONDAGEM E POÇO
        if float(CSV["Prof"][n_p]) < round(
            float(CSV["Topo_liso"][n_p])
            + float(CSV["C_liso"][n_p])
            + float(CSV["C_sec_filtr"][n_p]),
            4,
        ):
            lim = 60
            MSG = (
...
def cabecalho(file, CSV, n_p, existe_poco, i_layout, x, y, prof, VOC_dist, sist_geo):
    ...
    line(file, x + 0.3, 0, x + 0.3, prof)  # VOC vertical line
    ...
    rectangle(file, 0.02, X, y + prof + 0.42 + 0.3, X + 5, y + prof + 0.42 + 0.3 + 0.3)
    try:
        LAT = str(Lat).replace(",", "")
    ...
    mtext(file, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3, texto, subtexto)
    rectangle(file, 0.02, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3)
    ...
    mtext(file, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3, texto, subtexto)
    rectangle(file, 0.02, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3)
    line(file, X, y, X, y + prof + 0.42)

    # AMOSTRA
    X += 0.3
    texto = "AMOS."
    mtext(file, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3, texto)
    rectangle(file, 0.02, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3)
    line(file, X, y, X, y + prof + 0.42)

    try:
        insert_block(file, "CA", X + 0.3, prof - float(Prof_coleta) + y)
    except:
        try:
            Prof_coletas = Prof_coleta[1:-1].split(",")
            for i in Prof_coletas:
                insert_block(file, "CA", X + 0.3, prof - float(i) + y)
        except:
            pass
    ...
    rectangle(file, 0.02, X, y + prof + 0.42, X + 0.3, y + prof + 0.42 + 0.3)
    line(file, X, y, X, y + prof + 0.42)
    line(file, X + 0.15, y, X + 0.15, y + prof + 0.42)
    ...
    if not np.isnan(NA_sond):
        insert_block(file, "NA_SONDA", X + 0.15, prof - float(NA_sond) + y)
    ...
    if not np.isnan(NA_estab):
        insert_block(file, "NA_ESTAB", X + 0.30, prof - float(NA_estab) + y)

    # PERFIL DO POÇO
    X += 0.3
    texto = "PERFIL DO POÇO"
    mtext(file, X, y + prof + 0.42, X + 0.9, y + prof + 0.42 + 0.3, texto)
    rectangle(file, 0.02, X, y + prof + 0.42, X + 0.9, y + prof + 0.42 + 0.3)
    line(file, X, y, X, y + prof + 0.42)
    ...
    if isinstance(Local_da_sonda, str):
        texto = "  LOCAL DA SONDAGEM: " + str(Local_da_sonda)
    else:
        texto = "  LOCAL DA SONDAGEM: -"
    if len(texto) < 65:
        mtext(
            file,
            X,
    ...
    rectangle(file, 0.02, X, y + prof + 0.42 + 0.15, X + 2.9, y + prof + 0.42 + 0.3)
    line(file, X, y, X, y + prof + 0.42)

    # DIÂMETRO DA SONDAGEM
    if np.isnan(Diam_sond):
        texto = "  DIÂMETRO DA SONDAGEM: -"
    else:
        texto = "  DIÂMETRO DA SONDAGEM: " + str(Diam_sond).replace(".", ",") + "''"
    mtext(file, X, y + prof + 0.42, X + 1.45, y + prof + 0.42 + 0.15, texto, "", "left")
    rectangle(file, 0.02, X, y + prof + 0.42, X + 1.45, y + prof + 0.42 + 0.15)

    # DIÂMETRO DO POÇO
    if np.isnan(Diam_poco):
        texto = "  DIÂMETRO DO POÇO: -"
    else:
        texto = "  DIÂMETRO DO POÇO: " + str(Diam_poco).replace(".", ",") + "''"
    X += 1.45
    mtext(file, X, y + prof + 0.42, X + 1.45, y + prof + 0.42 + 0.15, texto, "", "left")
    rectangle(file, 0.02, X, y + prof + 0.42, X + 1.45, y + prof + 0.42 + 0.15)
    ...
    # SOLO NÍVEL 0
    if existe_poco == 1:
        rectangle(file, 0, x + 0.3, y + prof, x + 1.2 + 0.15 + 0.17, y + prof - 0.015)
        hatch(file, x + 0.6, y + prof, "0_LINHA", P="ANSI31 0.005 0 0")
        rectangle(
            file, 0, x + 2.1 - 0.15 - 0.17, y + prof, x + 0.3 + 4.7, y + prof - 0.015
        )
        hatch(file, x + 2.1, y + prof, "0_LINHA", P="ANSI31 0.005 0 0")
    else:
        rectangle(file, 0, x + 0.3, y + prof, x + 5, y + prof + 0.013)
        hatch(file, x + 0.6, y + prof, "0_LINHA", P="ANSI31 0.005 0 0")
    try:
        D = Diam_poco
        return D
...
def perfil_pedologico(file, n_p, x, y, D, Pedologia, prof, existe_poco):
    def solo(file, x, y, y1, y2, Diam_poco, string, existe_poco):
        ...
        if abs(y2 - y1) < 0.06:
            ...
            pontos = [x + 2.1, y1 + y, x + 5, y1 + y, x + 5, y2 + y, x + 2.1, y2 + y]
    ...
    def interpretar_cor(file, x, y, y2, description, existe_poco):
        ...
        def add_color(file, name, existe_poco, x, y, y2):
            ...
            if existe_poco == 1:
                hatch(file, x + 2.0, y2 + y, name, P="S")
            i = len(description)
            return i
            ...
            if description[i] in ["AMARELO", "AMARELA"]:
                if len(description) - 1 > i:
                    if description[i + 1] in ["ESCURO", "ESCURA"]:
                        i = add_color(
                            file, '"SC_AMARELO_ESCURO (Ocre)"', existe_poco, x, y, y2
                        )
                    else:
                ...
                if len(description) - 1 > i:
                    if description[i + 1] in ["CLARO", "CLARA"]:
                        ...
                        i = add_color(
                            file, '"SC_CINZA_ESVERDEADO"', existe_poco, x, y, y2
                        )
                    else:
                        i = add_color(file, '"SC_CINZA"', existe_poco, x, y, y2)
                else:
                    i = add_color(file, '"SC_CINZA"', existe_poco, x, y, y2)
            elif description[i] in (
                "CONCRETO",
                "CONCRETICO",
                "CONCRÉTICO",
                "(CONCRETO)",
                ...
                if len(description) - 1 > i:
                    if description[i + 1] in ["CLARO", "CLARA"]:
                        ...
                        i = add_color(
                            file, '"SC_MARROM_AVERMELHADO"', existe_poco, x, y, y2
                        )
                    elif description[i + 1] in ["ALARANJADA", "ALARANJADO"]:
                        i = add_color(
                            file, '"SC_MARROM_ALARANJADO"', existe_poco, x, y, y2
                        )
                    elif description[i + 1] in ["AMARELADA", "AMARELADO"]:
                        i = add_color(
                            file, '"SC_MARROM_AMARELADO"', existe_poco, x, y, y2
                        )
                    else:
                ...
                if len(description) - 1 > i:
                    if description[i + 1] in ["CLARO", "CLARA"]:
                        i = add_color(
                            file, '"SC_VERMELHO_CLARO"', existe_poco, x, y, y2
                        )
                    elif description[i + 1] in ["ESCURO", "ESCURA"]:
                        i = add_color(
                            file, '"SC_VERMELHO_ESCURO"', existe_poco, x, y, y2
                        )
                    elif description[i + 1] in ["ACINZENTADO", "ACINZENTADA"]:
                        i = add_color(
                            file, '"SC_VERMELHO_ACINZENTADO"', existe_poco, x, y, y2
                        )
                    else:
    ...
    def interpretar_granulometria(file, x, y, y2, descricao, existe_poco):
        ...
        def add_grain(file, existe_poco, x, y, y2, layer, pattern):
            ...
            if existe_poco == 1:
                hatch(file, x + 2.0, y2 + y, layer, P=pattern)
                ...
                if len(descricao) - 1 > i:
                    ...
                    if descricao[i + 1] in [
                        ...
                        if len(descricao) - 2 > i:
                            ...
                            add_grain(
                                file, existe_poco, x, y, y2, layer, "1areia+silte 1 0"
                            )
                        ...
                        if len(descricao) - 2 > i:
                            ...
                            add_grain(
                                file, existe_poco, x, y, y2, layer, "1areia+argila 1 0"
                            )
                    elif descricao[i + 1] in ["DE", "FORRAÇÃO"]:  # AREIA ARGILA
                        add_grain(
                            file, existe_poco, x, y, y2, "SC_AREIA_DE_FORRAÇÃO", "S"
                        )
                ...
                if len(descricao) - 1 > i:
                    if descricao[i + 1] in [
                        ...
                        if len(descricao) - 2 > i:
                            ...
                            add_grain(
                                file, existe_poco, x, y, y2, layer, "1silte+argila 1 0"
                            )
                ...
                if len(descricao) - 1 > i:
                    if descricao[i + 1] in [
                        ...
                        if len(descricao) - 2 > i:
                            ...
                            add_grain(
                                file, existe_poco, x, y, y2, layer, "1silte+argila 1 "
                            )
                    elif descricao[i + 1] in [
                        "ARENOSA",
                        "AREIA",
                        "ARENO",
                    ]:  # ARGILA AREIA
    ...
    try:
        ...
        for i in range(
            len(pedo[0::2])
        ):  # while estiver subindo pega y1 = 0 (primeiro) depois y1=y2
...
def adicionar_poco(file, CSV, n_p, prof, x, y, existe_poco):
    ...
    if existe_poco == 0:
        ...
        polyline(file, pontos, C="C")
        hatch(file, x + 1.65 - 0.025 * D - 0.05, prof - B_i + y, "H_BENTONITA", P="S")
        pontos = [
            x + 1.65 + 0.025 * D,
            prof - B_i + y,
            x + 1.65 + 0.025 * D + 0.1,
        ...
        polyline(file, pontos, C="C")
        hatch(file, x + 1.65 - 0.22, prof + y + CxConc, "H_CIMENTO", P="S")
        pontos = [
            x + 1.65 + 0.30,
            prof + y,
            x + 1.65 + 0.29,
        ...
        hatch(file, x + 1.65, prof - (T_l + 0.025 * D) + y, "1_BORRACHA", P="S")
        rectangle(
            file,
            0,
            x + 1.65 - 0.025 * D,
        ...
        hatch(file, x + 1.65, prof - (T_l + 0.027 * D) + y, "1_ACABAMENTO", P="S")
        rectangle(
            file,
            0,
            x + 1.65 - 0.008 * D,
        ...
        hatch(file, x + 1.65, prof - (T_l + 0.029 * D) + y, "1_ACABAMENTO", P="S")
        rectangle(
            file,
            0,
            x + 1.65 - 0.006 * D,
        ...
        hatch(file, x + 1.65, prof - (T_l + 0.055 * D) + y, "1_ACABAMENTO", P="S")
        hatch(file, x + 1.65, prof - (T_l + 0.055 * D) + y, "0_LINHA", P="LINE 0.001 0")
        pontos = [
            x + 1.65 - 0.006 * D,
            prof - (T_l + 0.055 * D) + y,
            x + 1.65 - 0.005 * D,
        ...
        polyline(file, pontos, "C", L="0_LINHA")
        hatch(file, x + 1.65, prof - (T_l + 0.057 * D) + y, "1_ACABAMENTO", P="S")
        ...
        if PescComp > 0:
            ...
            polyline(file, pontos, "C", L="0_LINHA")
            hatch(file, x + 1.65 - 0.14, prof + y + 0.04, "1_ACABAMENTO", P="S")
            pontos = [
                x + 1.65 - 0.13,
                prof + y + PescComp,
                x + 1.65 - 0.13,
            ...
            polyline(file, pontos, "C", L="0_LINHA")
            hatch(
                file, x + 1.65 - 0.14, prof + y + PescComp + 0.01, "1_ACABAMENTO", P="S"
            )

            pontos = [
                x + 1.65 - 0.16,
                prof + y + PescComp - 0.005,
                x + 1.65 - 0.16 - 0.013,
                prof + y + PescComp + 0.005,
                x + 1.65 - 0.16 - 0.013,
                prof + y + PescComp + 0.025,
                x + 1.65 - 0.14,
                prof + y + PescComp + 0.032,
                x + 1.65 - 0.14,
                prof + y + PescComp + 0.01,
                x + 1.65 - 0.16,
                prof + y + PescComp + 0.01,
            ...
            polyline(file, pontos, "C", L="0_LINHA")
            hatch(
                file, x + 1.65 - 0.14, prof + y + PescComp - 0.02, "1_ACABAMENTO", P="S"
            )
            circle(
                file, x + 1.65 - 0.17, prof + y + PescComp + 0.02, R=0.015, L="0_LINHA"
            )
            hatch(
                file,
                x + 1.65 - 0.17,
                prof + y + PescComp + 0.035,
                "1_ACABAMENTO",
                P="S",
            )
            ...
            pontos = [
                ...
                prof + y + PescComp - 0.01,
                x + 1.65 + 0.17,
                prof + y + PescComp - 0.05,
                x + 1.65 + 0.21,
                prof + y + PescComp - 0.05,
                x + 1.65 + 0.21,
                prof + y + PescComp - 0.04,
                x + 1.65 + 0.18,
                prof + y + PescComp - 0.04,
                x + 1.65 + 0.17,
                prof + y + PescComp + 0.005,
            ...
            hatch(
                file,
                x + 1.65 + 0.21,
                prof + y + PescComp - 0.045,
                "1_ACABAMENTO",
                P="S",
            )
            circle(
                file, x + 1.65 + 0.21, prof + y + PescComp - 0.045, R=0.015, L="0_LINHA"
            )
            hatch(
                file,
                x + 1.65 + 0.21,
                prof + y + PescComp - 0.045 + 0.015,
                "1_ACABAMENTO",
                P="S",
            )
            circle(file, x + 1.65 + 0.16, prof + y + PescComp, R=0.015, L="0_LINHA")
            hatch(
                file,
                x + 1.65 + 0.16 + 0.015,
                prof + y + PescComp,
            ...
            hatch(file, x + 1.65, prof + y + PescComp + 0.04, "1_ACABAMENTO", P="S")

            pontos = [
                x + 1.65 + 0.09,
                prof + y + PescComp + 0.01 + 0.03,
                x + 1.65 + 0.11,
                prof + y + PescComp + 0.01 + 0.05,
                x + 1.65 + 0.125,
                prof + y + PescComp + 0.01 + 0.05,
                x + 1.65 + 0.125,
                prof + y + PescComp + 0.01 + 0.03,
            ...
            hatch(
                file,
                x + 1.65 + 0.125,
                prof + y + PescComp + 0.01 + 0.04,
                "1_ACABAMENTO",
                P="S",
            )

            circle(
                file, x + 1.65 + 0.125, prof + y + PescComp + 0.06, 0.015, L="0_LINHA"
            )
            hatch(
                file,
                x + 1.65 + 0.140,
                prof + y + PescComp + 0.06,
                "1_ACABAMENTO",
                P="S",
            )
            ...
            rectangle(
                ...
                prof + abs(T_l) + 0.10 + y,
                x + 1.65 + 0.13,
                prof + abs(T_l) + 0.10 + 0.0132 + y,
            )
            hatch(
                file, x + 1.65 - 0.12, prof + abs(T_l) + 0.10 + y, "1_ACABAMENTO", P="S"
            )
            line(
                file,
                x + 1.65 - 0.13,
            ...
            y = y + CxConc
            rectangle(
                file, 0, x + 1.65 - 0.13, prof + y, x + 1.65 + 0.13, prof - 0.01 + y
            )
            ...
            rectangle(
                ...
                prof - 0.013 + y,
                x + 1.65 - 0.125,
                prof - 0.013 - T_l - round(0.75 * (C_f - T_l), 4) + y,
            )
            hatch(file, x + 1.65 - 0.125, prof - 0.014 + y, "1_BRANCO", P="S")
            rectangle(
                file,
                0,
                x + 1.65 + 0.13,
            ...
            pontos = [
                ...
                prof - 0.015 + y,
                x + 1.65 - 0.148,
                prof - 0.016 + y,  # inicio_curva
                x + 1.65 - 0.1465,
                prof - 0.017 + y,  # inicio_curva
                x + 1.65 - 0.145,
                prof - 0.0185 + y,
                x + 1.65 - 0.1435,
                prof - 0.0202 + y,
                x + 1.65 - 0.142,
                prof - 0.0225 + y,  # fim_curva
                x + 1.65 - 0.141,
                prof - 0.025 + y,  # fim_curva
                x + 1.65 - 0.14,
                prof - 0.03 + y,
                x + 1.65 - 0.14,
                prof - 0.15 + y,
            ...
            polyline(file, pontos, "C", L="0_LINHA")
            hatch(file, x + 1.65 - 0.14, prof - 0.15 + y, "1_ACABAMENTO", P="S")
            pontos = [
                x + 1.65 + 0.13,
                prof + y,
                x + 1.65 + 0.19,
...
def fazer_layout(file, layout, titulo, existem_pocos, CTB, sist_geo, logo_png):
    ...
    def perfil_esquematico_poco(file):
        ...
        circle(file, 262 - 0.175 * D, 170 + 0.06 * D, 0.03 * D, L="0_LINHA")
        hatch(file, 262 - 0.175 * D, 170 + 0.06 * D + 0.03 * D, "1_BRANCO", P="S")
        line(
            file,
            262 - 0.23 * D,
            170 + 0.105 * D,
        ...
        rectangle(file, 0, 262 - 0.25 * D, 170, 262 + 0.25 * D, 170 - (0.25 * D))
        hatch(file, 262, 170 - 0.25 * D, "1_BORRACHA", P="S")
        rectangle(
            file, 0, 262 - 0.25 * D, 170 - (0.25 * D), 262 + 0.25 * D, 170 - (0.27 * D)
        )
        hatch(file, 262, 170 - 0.27 * D, "1_ACABAMENTO", P="S")
        rectangle(
            file, 0, 262 - 0.08 * D, 170 - (0.27 * D), 262 + 0.08 * D, 170 - (0.29 * D)
        )
        hatch(file, 262, 170 - 0.29 * D, "1_ACABAMENTO", P="S")
        rectangle(
            file, 0, 262 - 0.06 * D, 170 - (0.29 * D), 262 + 0.06 * D, 170 - (0.55 * D)
        )
        hatch(file, 262, 170 - (0.55 * D), "1_ACABAMENTO", P="S")
        hatch(file, 262, 170 - (0.55 * D), "0_LINHA", P="LINE 0.04 0")
        pontos = [
            262 - 0.06 * D,
            170 - (0.55 * D),
            262 - 0.05 * D,
        ...
        polyline(file, pontos, "C", L="0_LINHA")
        hatch(file, 262, y + 170 - (0.57 * D), "1_ACABAMENTO", P="S")

        # ACABAMENTO
        rectangle(file, 0, 262 - 2.7, 173 - 0.7, 262 - 2.5, 168.5)
        ...
        def seta(file, x, y, txt):
            ...
            line(file, x, y, 271, y)
            line(file, x, y, x + 1.4, y_arrow + 0.5)
            line(file, x, y, x + 1.4, y_arrow - 0.5)

        x_arrow, y_arrow = [265.5, 172.7]
        txt = "Câmara de calçada"
        ...
        mtext(
            ...
            285.1810,
            178.9105 + 8,
            "PERFIL ESQUEMÁTICO DE UM POÇO MONITORAMENTO",
            J="left",
            H=1.6694,
            R=0,
        )
    ...
    def legenda(file):
        ...
        mtext(
            ...
            53.6600,
            10.8758 + 4 * gap,
            "Nível da água durante a sondagem",
            J="left",
            H=1.44,
            R=0,
        )
        insert_block(
            file, "NA_SONDA", 10.8758, 10.8758 + 3.5 * gap, scale_x="20", scale_y="20"
        )
        mtext(
            file,
            12.7863,
            ...
            53.6600,
            10.8758 + 3 * gap,
            "Nível da água estabilizado",
            J="left",
            H=1.44,
            R=0,
        )
        insert_block(
            file, "NA_ESTAB", 10.8758, 10.8758 + 2.5 * gap, scale_x="20", scale_y="20"
        )
        ...
        mtext(
            ...
            53.6600 + 0.3,
            10.8758 + gap,
            "Profundidade do ponto amostrado",
            J="left",
            H=1.44,
            R=0,
        )
        insert_block(
            file, "CA", 10.8758, 10.8758 + 0.5 * gap + 0.3, scale_x="20", scale_y="20"
        )
    ...
    if existem_pocos > 0:
        ...
        rectangle(
            file, 0, 6.1966, 27.3131, 251.3136, 196.6754, L="0_SELO"
        )  # Retangulo pro Viewport menor
        texto = "FIGURA XX – PERFIS DE SONDAGEM E POÇOS DE MONITORAMENTO " + titulo
        mtext(
            file,
            53.6600 + 2.7814,
            19.5444,
        ...
        file.write(
            "_VIEWPORTS 6.1966,27.3131 289.1967,196.6754\n"
        )  # Cria o viewport maior
        rectangle(
            file, 0, 6.1966, 27.3131, 289.1967, 196.6754, L="0_SELO"
        )  # Retangulo pro Viewport maior
        texto = "FIGURA XX – PERFIS DE SONDAGEM " + titulo
        mtext(
            file,
            53.6600 + 2.7814,
            19.5444,
    ...
    rectangle(
        file, 0, 6.1966, 196.6754, 289.1967, 11.7758, L="0_SELO"
    )  # Retangulo pro layout
    rectangle(
        file, 0, 6.1966, 27.3131, 289.1967, 11.7758, L="0_SELO"
    )  # Retangulo pro rodape

    line(
        file, 53.6600, 27.3131, 53.6600, 11.7758, L="0_SELO"
    )  # Linha nome figura vertical
    line(
        file, 53.6600, 19.5444, 241.1388, 19.5444, L="0_SELO"
    )  # Linha nome figura horizontal

    line(
        file, 218.3075, 19.5444, 218.3075, 27.3131, L="0_SELO"
    )  # Linha escala vertical
    texto = escala
    mtext(
        file,
        218.3075 + 2.7814,
    ...
    line(
        file, 127.7329, 11.7758, 127.7329, 19.5444, L="0_SELO"
    )  # Linha cliente vertical
    texto = cliente
    mtext(
        file,
        127.7329 + 2.7814,
    ...
    line(
        file, 168.3090, 11.7758, 168.3090, 19.5444, L="0_SELO"
    )  # Linha sist_geo vertical
    texto = sist_geo
    gap = 0
    if len(sist_geo) > 25:
        gap = 1
...
cliente, projeto, data, escala, sist_geo, CSV, logo_png, output_scr_acad = read_files(
    title_block, borehole_data, output_scr_acad, logo_png
)
...
# 4. Open output file
file = codecs.open(output_scr_acad, "wb", encoding="cp1252")
file.write("OSMODE 0\n")
...
for n_p in normal:
    ...
    D = cabecalho(
        file, CSV, n_p, existe_poco, i_layout, x, y, prof_max, VOC_dist, sist_geo
    )
    ...
    if i_layout == 0:  # 1st borehole
        ...
        mspace_zoom(file, x - 0.05, y - 0.05, x + 5.18 + 0.05, y + prof_max + 1.02)
        file.write("PSPACE\nMODEL\n")
        x = x + 5.18
        i_layout += 1
    else:  # 2nd borehole
        file.write("-LAYOUT S " + layout + "\n")  # To the layout
        mspace_zoom(
            file, x - 5.18 - 0.05, y - 0.05, x + 5.0 + 0.05, y + prof_max + 1.02
        )
        file.write("PSPACE\nMODEL\n")
        x = x + 11.23
        i_layout = 0
...
for n_p in shallow:
    ...
    if i_layout == 0:  # 1st borehole
        ...
        file.write("-LAYOUT S " + layout + "\n")  # To the layout
        mspace_zoom(
            file, x - 5.18 - 0.05, y - 0.05, x + 5.00 + 0.05, y + prof_max + 1.02 + 0.05
        )
        file.write("PSPACE\nMODEL\n")
        x = x - 5.18
        y = y - (prof_max + 0.42 + 0.3 + 0.3 + 0.18)
        i_layout += 1
...
# 4.3 Final message
output_scr_acad = os.path.basename(output_scr_acad)

division(84)
msg = (
    'Done! Open AutoCAD, select "No Template-Metric", click on "Start drawing!"\n'
    'Type "scr", press enter, and select "' + output_scr_acad + '"!'
)

signature = (
    "Automatically Soil Profile Drawer\n"
    "Last update 2024\n"
    "----------------\n"
